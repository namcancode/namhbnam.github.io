...
module.exports = {
	...
	readJsonObjectFromFile: fileName => {
		fs.readFile(fileName, (err, data) => {
			...
			console.log(
				`jsonObject.resultCode = ${JSON.stringify(jsonObject.resultCode)}`
			);
			console.log(
				`jsonObject.restaurantName = ${JSON.stringify(
					jsonObject.restaurantName
				)}`
            );
            console.log(
				`jsonObject.address = ${JSON.stringify(
					jsonObject.address
				)}`
			);
		});
